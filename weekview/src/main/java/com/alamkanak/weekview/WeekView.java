...
public class WeekView extends View {
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        if (event.getName() != null) {
            ...
            if (event.getmNameColor().equals("Black")){
                final ForegroundColorSpan fcs = new ForegroundColorSpan(Color.parseColor("#000000"));
                bob.setSpan(fcs, 0, bob.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
            }else if (event.getmNameColor().equals("White")){
                final ForegroundColorSpan fcs = new ForegroundColorSpan(Color.parseColor("#FFFFFF"));
                bob.setSpan(fcs, 0, bob.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
            }
